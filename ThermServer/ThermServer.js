...
var termDBFunction = require('./routes/termDB');
var termDBStatFunction = require('./routes/termStatDB');
var termManagment = require('./routes/termManagment');
var app = express();
...
var accessLogStream = fs.createWriteStream(logPath+'/ThermServer.log', { flags: 'a' });
app.use(morgan(
	':date :res[content-length] :remote-addr :method :url - RC: :status :response-time ms', { stream: accessLogStream }));
...
{
  termDBFunction.DBInitialize(termStatdb);
}
...
app.get('/rest/checkConfigurationChange/:key', termManagment.checkConfigurationChange);
...
app.post('/rest/updateTempProgramming',  urlencodedParser,termManagment.updateTempProgramming);
// Applicazione WEB
...
app.get('/', function (req, res) {
  ...
  if (newPath.slice(-1) == "/")
	res.redirect(newPath+'dashboard');
  else
	res.redirect(newPath+'/dashboard');
});
